import { Checkout, EmptyCart, Gotoyourcart, Href, ImagePath, OrderTotal } from "@/Constants";
      ...
      {items.length > 0 ? (
        <>
          {items.map((item) => (
            <li className="cart-product" key={item.id}>
              <div className="d-flex">
                <img className="img-fluid b-r-5 me-3 img-60" src={`${ImagePath}/other-images/cart-${item.image}`} alt="cosmetic" />
                <div className="flex-grow-1">
                  <span className="f-w-500">{item.name}</span>
                  <div className="qty-box">
                    <div className="touchspin-wrapper d-flex flex-row">
                      <Button className="decrement-touchspin btn-touchspin" onClick={() => handleDecrement(item.id)}>
                        <Minus />
                      </Button>
                      <Input className="input-touchspin" id="inputData" type="number" value={item.quantity} readOnly />
                      <Button className="increment-touchspin btn-touchspin" onClick={() => handleIncrement(item.id)}>
                        <Plus />
                      </Button>
                    </div>
          ...
          <li className="text-center">
            <Link className="d-block mb-3 view-cart f-w-700" href={Href}>
              {Gotoyourcart}
            </Link>
            <Link className="btn btn-primary view-checkout w-100" href={Href}>
              {Checkout}
            </Link>
          </li>
        </>
        ...
        <div className={`cart-empty ${items.length === 0 ? "show" : ""}`}>
          <div className="cart-image">
            <Image height={60} width={60} className="img-fluid" src={`${ImagePath}/product/order-trash.gif`} alt="empty" />
          </div>
          <h5>{EmptyCart}</h5>
        </div>
      )}
